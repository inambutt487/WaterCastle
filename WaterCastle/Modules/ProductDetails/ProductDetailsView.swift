    ...
    fileprivate func SimilerProduct() -> some View {
        return VStack(alignment: .leading) {
            ...
            ScrollView(.horizontal, showsIndicators: false, content: {
                HStack(spacing: 10) {
                    ForEach(self.arrCloth) { product in
                        ProductView(cloth: product)
                    }
                }.padding(.leading, 15)
            })
        }.background(Color.white)
...
struct ItemDetailsView_Previews: PreviewProvider {
    static var previews: some View {
        ProductDetailsView(show: .constant(false), cloth: Product(name: "Women Printed Flare Dress", description: "Green printed woven fit and flare dress, has a notched lapel collar and sleevesless.", imageURL: "greenDress", price: 2149, company: "Chemistry Edition", rating: 4, type: "sale", isFavorite: false, color: "Green", size: "L", discount: 40))
    }
}
